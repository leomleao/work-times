...
import type { RetrySyncRunRequestBody, RetrySyncRunResponseBody, RunRequest } from '$lib/server/sync/contracts';
...
export const POST: RequestHandler = async ({ locals, params, request }) => {
  let body: RetrySyncRunRequestBody & { csrfToken?: string } = {};
  try {
  ...
  const idempotencyKey = `retry-${id}-${targetDate ?? 'all'}-${Date.now()}`;
